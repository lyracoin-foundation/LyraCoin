...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: LyraCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: LyraCoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 49840 or testnet: 45544)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 49841 or testnet: 45644)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeLyraCoinamount=<n> " + _("Keep N LyraCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LyraCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LyraCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LyraCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeLyraCoinAmount = GetArg("-anonymizeLyraCoinamount", 0);
    if(nAnonymizeLyraCoinAmount > 999999) nAnonymizeLyraCoinAmount = 999999;
    if(nAnonymizeLyraCoinAmount < 2) nAnonymizeLyraCoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeLyraCoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize LyraCoin Amount %d\n", nAnonymizeLyraCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1LyraCoin+1000 == (.1LyraCoin+100)*10
       10LyraCoin+10000 == (1LyraCoin+1000)*10
    */
