...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLyraCoinAmount"))
        settings.setValue("nAnonymizeLyraCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLyraCoinAmount = settings.value("nAnonymizeLyraCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLyraCoinAmount"))
        SoftSetArg("-anonymizeLyraCoinamount", settings.value("nAnonymizeLyraCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeLyraCoinAmount:
            nAnonymizeLyraCoinAmount = value.toInt();
            settings.setValue("nAnonymizeLyraCoinAmount", nAnonymizeLyraCoinAmount);
            emit anonymizeLyraCoinAmountChanged(nAnonymizeLyraCoinAmount);
            break;
        default:
            break;
        }
